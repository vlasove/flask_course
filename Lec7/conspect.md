...
### Небольшое задание
Как известно, для реализации базового интерфейса обращения с любым объектом необходимо прописать правила, которые называются ***CRUD***:
* ***Create*** (это уже есть)
* ***Read/Retrive*** (это тоже есть в виде списка и детальный)
* ***Update*** (этого еще нет)
* ***Delete*** (этого тоже нет)

***Задачка*** реализовать функционал Update и Delete для объектов модели Post.
Для этого вспользуйтесь следующими подсказами.
* ```posts/<id>/update``` - url запрос для показа формы обновления поста с id=```id```. После успешного обновления - редирект на ```detail_view``` поста с id=```id```
* ```posts/<id>/delete``` - url запрос для показа подтверждения удаления поста с id =```id```(на странице написано что-то вроде ***Are u sure want to  delete thois post?*** и Кнопка подтверждения).
После успешного удаления - редирект на ```posts_list```.

Условимся, что оба запроса облагаются декоратором ```@login_required```